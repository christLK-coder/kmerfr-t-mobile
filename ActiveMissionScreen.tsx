...
export default function ActiveMissionScreen({ navigation }: Props) {
  ...
  const startTracking = () => {
    ...
    TelemetryService.startTracking(mission.id, {
      onHazard: (severity) => {
        ...
        Animated.timing(shockAnim, { toValue: 0, duration: 900, useNativeDriver: true }).start();
      },
  ...
  return (
    <View style={s.root}>
      ...
      {/* Stats */}
      <View style={[s.statsBar, { backgroundColor: bgColor }]}>
      ...
      {/* Actions */}
      <View style={[s.actions, { paddingBottom: Math.max(insets.bottom, 14), backgroundColor: bgColor }]}>
        ...
        <Pressable style={[s.btn, { backgroundColor: C.surfaceVariant }]}
          onPress={() => Alert.alert(t('drive_pause'), '', [{ text: t('common_cancel') }])}>
          <MaterialCommunityIcons name="pause-circle-outline" size={20} color={C.textPrimary} />
          <Text style={[s.btnTxt, { color: C.textPrimary }]}>{t('drive_pause')}</Text>
        </Pressable>
