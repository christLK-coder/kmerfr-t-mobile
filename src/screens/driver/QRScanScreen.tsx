...
export default function QRScanScreen({ route, navigation }: Props) {
  ...
  useEffect(() => {
    ...
    const loop = Animated.loop(
      Animated.sequence([
        Animated.timing(scanLineY, { toValue: FRAME - 8, duration: 1800, easing: Easing.linear, useNativeDriver: true }),
        Animated.timing(scanLineY, { toValue: 0, duration: 1800, easing: Easing.linear, useNativeDriver: true }),
      ])
    );
    loop.start();
    return () => loop.stop();
        ...
        <View style={styles.scanArea}>
          ...
          <View style={styles.maskMiddle}>
            ...
            <Animated.View style={[styles.scanFrame, { transform: [{ scale: frameScale }] }]}>
              ...
              {/* Ligne de scan animée — visible seulement si pas en train de valider */}
              {!completing && !confirmed && (
                <Animated.View
                  style={[styles.scanLine, { backgroundColor: C.primary, transform: [{ translateY: scanLineY }] }]}
                />
              )}

              {/* Spinner de validation */}
              {completing && !confirmed && (
...
const styles = StyleSheet.create({
  ...
  scanLine: {
    position: 'absolute', left: 8, right: 8, height: 2, borderRadius: 1, opacity: 0.8,
  },
