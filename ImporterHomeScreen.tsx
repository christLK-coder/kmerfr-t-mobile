  ...
  return (
    <View style={[s.root, { backgroundColor: C.background }]}>
      ...
      <LinearGradient colors={[C.gradientPrimaryStart, C.gradientPrimaryEnd]} style={[s.header, { paddingTop: insets.top + 8 }]}>
        ...
        {/* Stats rapides — cards animées */}
        <View style={s.statsRow}>
          {[
            { icon: 'truck-fast', num: activeMissions.filter(m => m.status === 'IN_TRANSIT').length, label: t('status_IN_TRANSIT'), color: '#FF6D00' },
            { icon: 'clock-outline', num: activeMissions.filter(m => m.status === 'ASSIGNED').length, label: t('status_ASSIGNED'), color: '#FFAB40' },
            { icon: 'check-circle', num: historyMissions.filter(m => m.status === 'DELIVERED').length, label: t('status_DELIVERED'), color: '#81C784' },
          ].map((stat, i) => (
            <View key={i} style={s.statCard}>
              <View style={[s.statIconBg, { backgroundColor: stat.color + '33' }]}>
        ...
        <View style={s.spendingCard}>
          <View style={s.spendingLeft}>
            <Text style={s.spendingLabel}>Total depenses</Text>
