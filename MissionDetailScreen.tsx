...
export default function MissionDetailScreen() {
  ...
  const load = useCallback(async () => {
    try {
      ...
      setMission(m);
      // Si IN_TRANSIT, s'abonner au tracking WebSocket
      if (m.status === 'IN_TRANSIT') subscribeTracking();
    } catch (e) {
  ...
  useEffect(() => {
    load();
    return () => { if (wsSubId) WebSocketService.unsubscribe(wsSubId); };
  }, []);
  ...
  const subscribeTracking = () => {
    try {
      ...
      const id = WebSocketService.subscribe(
        ...
        (body: any) => setDriverPos({ lat: body.lat, lng: body.lng, speed: body.speed, heading: body.heading })
      );
      setWsSubId(id);
    } catch {}
  };
  ...
  return (
    <View style={[s.root, { backgroundColor: C.background }]}>
      ...
      >
        ...
        {mission.driverName && (
          <View style={[s.card, { backgroundColor: C.surface }]}>
            <Text style={[s.sectionTitle, { color: C.textSecondary }]}>Transporteur</Text>
            <View style={s.driverRow}>
              <View style={[s.driverAvatar, { backgroundColor: C.primaryContainer }]}>
                <Text style={[s.driverInitials, { color: C.primary }]}>
                  {mission.driverName.charAt(0).toUpperCase()}
                </Text>
              </View>
              <View style={{ flex: 1 }}>
                <Text style={[s.driverName, { color: C.textPrimary }]}>{mission.driverName}</Text>
                {mission.driverPhone && (
              ...
              <Pressable
                ...
                onPress={() => nav.navigate('Chat', {
                  missionId,
                  otherPartyName: mission.driverName ?? 'Chauffeur',
                })}
              >
        ...
        {/* DELIVERED + paiement pas encore RELEASED → Payer le solde */}
        {isDelivered && mission.paymentStatus !== 'RELEASED' && (
          <Button
            mode="contained"
            onPress={() => {
...
const s = StyleSheet.create({
  ...
  mapCard: { borderRadius: 14, overflow: 'hidden', elevation: 1, padding: 12 },
  mapContainer: { height: 220, borderRadius: 10, overflow: 'hidden', marginTop: 8 },
