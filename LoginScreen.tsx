  ...
  return (
    ...
    >
      ...
      >
        ...
        {pendingAppId && (
          <Pressable
            style={[s.trackCard, { backgroundColor: '#E3F2FD', borderColor: '#0277BD30' }]}
            onPress={() => nav.navigate('DriverOnboarding', { applicationId: pendingAppId })}
          >
            <View style={[s.trackIcon, { backgroundColor: '#0277BD20' }]}>
              <MaterialCommunityIcons name="file-document-check-outline" size={24} color="#0277BD" />
            </View>
            <View style={{ flex: 1 }}>
              <Text style={s.trackTitle}>Candidature en cours</Text>
              <Text style={s.trackSub}>Suivre l'avancement et envoyer des documents</Text>
            </View>
            <MaterialCommunityIcons name="chevron-right" size={20} color="#0277BD" />
          </Pressable>
        )}
      </Animated.ScrollView>
    </KeyboardAvoidingView>
...
const s = StyleSheet.create({
  ...
  trackTitle: { fontSize: 14, fontWeight: '700', color: '#0277BD' },
  trackSub: { fontSize: 11, color: '#0277BD', marginTop: 2, opacity: 0.7 },
});
