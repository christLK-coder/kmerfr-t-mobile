...
export default function AdminDashboardScreen() {
  ...
  const load = async () => {
    try {
      ...
      console.warn('[AdminDash] Erreur stats :', e?.response?.status, e?.message);
  ...
  return (
    <View style={[s.root, { backgroundColor: C.background }]}>
      ...
      <ScrollView
        contentContainerStyle={[s.content, { paddingBottom: insets.bottom + 24, maxWidth: layout.contentMaxWidth, alignSelf: 'center', width: '100%' }]}
      ...
      >
        ...
        <View style={s.quickGrid}>
          ...
          <QuickAction icon="lock-open-variant-outline" label="Séquestres" color={C.primary}
            onPress={() => nav.navigate('AdminOperations' as any)} C={C} />
          <QuickAction icon="cash-multiple" label="Finances" color={C.success}
        ...
        <View style={s.quickGrid}>
          ...
          <QuickAction icon="cog-outline" label="Opérations" color={C.textSecondary}
            onPress={() => nav.navigate('AdminOperations' as any)} C={C} />
        </View>
      </ScrollView>
    </View>
