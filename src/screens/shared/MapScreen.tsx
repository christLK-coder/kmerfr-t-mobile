...
const STATUS_COLORS: Record<string, string> = {
  OPEN: '#00897B', ASSIGNED: '#F57F17', IN_TRANSIT: '#E65100', DELIVERED: '#2E7D32',
};

// Échappe une chaîne pour l'injection sûre dans un littéral JS entre guillemets simples
...
function buildLeafletHtml(
  ...
  const missionMarkers = missions
    .filter(m => m.originLat != null && m.destinationLat != null)
    .map(m => {
      const color  = STATUS_COLORS[m.status] ?? '#757575';
      const origin = jsStr(m.originLabel);
      const dest   = jsStr(m.destinationLabel);
      const price  = (m.totalPrice ?? 0).toLocaleString('fr-FR');
      return `
      L.circleMarker([${m.originLat},${m.originLng}],{radius:9,fillColor:'${color}',color:'#fff',weight:2.5,fillOpacity:0.95})
        .addTo(map).bindPopup('<b>${origin}</b><br><span style="color:#666">&#8594; ${dest}</span><br><span style="color:${color};font-weight:700">${m.status}</span><br><b style="color:${primaryColor}">${price} FCFA</b>');
      L.circleMarker([${m.destinationLat},${m.destinationLng}],{radius:6,fillColor:'${color}',color:'#fff',weight:2,fillOpacity:0.5})
        .addTo(map);
      L.polyline([[${m.originLat},${m.originLng}],[${m.destinationLat},${m.destinationLng}]],
        {color:'${color}',weight:2.5,opacity:0.5,dashArray:'8,10'}).addTo(map);`;
    }).join('\n');
...
export default function MapScreen() {
  ...
  // Rebuild uniquement quand GPS ET les missions sont prêts
  useEffect(() => {
    if (!gpsReady) return;
    setHtmlContent(
      buildLeafletHtml(activeMissions, userLat, userLng, isDark, C.primary)
    );
  }, [gpsReady, activeMissions, userLat, userLng, isDark]);
