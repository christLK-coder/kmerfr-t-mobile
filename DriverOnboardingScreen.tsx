...
const STATUS_CONFIG: Record<string, { icon: string; color: string; label: string; bg: string }> = {
  ...
  UNDER_REVIEW: { icon: 'magnify',             color: '#0277BD', label: 'En vérification',   bg: '#E3F2FD' },
          ...
          renderItem={({ item }) => {
            ...
            return (
              <View style={[st.msgRow, isMe && st.msgRowMe]}>
                {!isMe && (
                  <View style={[st.msgAvatar, { backgroundColor: '#0277BD20' }]}>
                    <MaterialCommunityIcons name="shield-account" size={16} color="#0277BD" />
                  </View>
                )}
                <View style={[
                  st.msgBubble,
                  ...
                  {!isMe && <Text style={[st.msgSender, { color: '#0277BD' }]}>Admin KmerFret</Text>}
                  ...
                  /* === FICHIER AUTRE AVEC URL === */
                    ...
                    <Pressable onPress={() => Linking.openURL(fullUrl)} style={st.pdfCard}>
                      <View style={[st.pdfIconBox, { backgroundColor: '#E3F2FD' }]}>
                        <MaterialCommunityIcons name="file-document-outline" size={28} color="#0277BD" />
                      </View>
                      <View style={{ flex: 1 }}>
                        <Text style={[st.pdfName, { color: isMe ? '#fff' : C.textPrimary }]} numberOfLines={2}>
                          {displayName}
