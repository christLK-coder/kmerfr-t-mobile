...
export async function saveSession(session: LocalSession): Promise<void> {
  await db.runAsync(
    `INSERT OR REPLACE INTO local_session
       (id, user_id, user_role, jwt_token, refresh_token, full_name, email, phone,
        biometric_expires_at, expires_at, updated_at)
    ...
    [
      ...
      session.email ?? null,
      session.phone ?? null,
      session.biometric_expires_at ?? null,
      session.expires_at,
    ]
  );
}
...
export async function getSession(): Promise<LocalSession | null> {
  return db.getFirstAsync<LocalSession>('SELECT * FROM local_session WHERE id = 1');
}
